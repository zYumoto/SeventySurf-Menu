...
export function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#091112]/78 backdrop-blur-xl">
        ...
        <nav className="hidden items-center gap-1 lg:flex">
          {navItems.map((item) => (
            <Link
              ...
              to={item.href}
              className={cn(
                "rounded-md px-4 py-2 text-sm font-semibold text-white/70 transition hover:bg-white/8 hover:text-white",
          ...
          <nav className="container grid gap-2">
            {navItems.map((item) => (
              <Link
                ...
                className={cn(
                  "rounded-lg px-4 py-3 text-base font-semibold text-white/72",
                  pathname === item.href && "bg-white/10 text-white"
                )}
              >
                {item.label}
